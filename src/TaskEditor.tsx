...
import { statuses, type HalfDay, type Project, type Task } from './model';
...
export function TaskEditor({ task, project, scheduled, onChange, onOrder, onDependency, onDelete, onClose }: {
  task: Task; project: Project; scheduled?: Scheduled;
  onChange: (task: Task) => void; onOrder: (order: number) => void;
  ...
  return <aside className="editor" aria-label="任务详情编辑">
    ...
    <div className="field-label">前置依赖任务</div>
    ...
    <div className="inline"><select aria-label="选择前置任务" value={dependency} onChange={e => setDependency(e.target.value)}><option value="">选择前置任务…</option>{project.tasks.filter(t => t.uid !== task.uid && !task.dependencies.includes(t.uid)).map(t => <option key={t.uid} value={t.uid}>{t.order}. {t.name}</option>)}</select><button disabled={!dependency} onClick={() => { onDependency(dependency, task.uid); setDependency(''); }}>添加</button></div>
    ...
    <button className="danger-text full-width" onClick={onDelete}>删除任务</button>
  </aside>;
}
