...
export function App() {
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => { if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 's') { event.preventDefault(); (document.activeElement as HTMLElement)?.blur(); setTimeout(() => void save(), 0); } };
  ...
  if (!project) return <div className="loading"><span className="brand-icon">M</span><h1>MissionManager</h1><p>{fileError || '正在读取项目 YAML…'}</p><p className="muted">启动文件无效时，修复 YAML 后会自动重试。</p></div>;
    ...
    <main>
      ...
      <fieldset className="workspace-fieldset" disabled={saving}>
        <section className="board">
          ...
          <div className="board-footer"><div className="legend">{statuses.map(s => { const color = project.project.status_colors[s]; return <span key={s}><i style={{ backgroundColor: color.fill, borderColor: color.border }} />{s}</span>; })}<span><i className="late-key" />超期</span></div><span>未匹配任务保留原行 · 选中任务显示依赖连线</span></div>
        </section>
