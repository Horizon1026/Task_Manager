...
type Props = { project: Project; schedule: Map<string, Scheduled>; scale: Scale; selected: string | null; filter: { labels: string[]; mode: 'and' | 'or' }; focusUid: string | null; canFocus: boolean; onFocusChange: (uid: string | null) => void; onSelect: (uid: string | null) => void; onChange: (task: Task) => void; onOrder: (uid: string, order: number) => void; onSiblingOrder: (uid: string, targetUid: string) => void; onToggleCollapse: (uid: string) => void; relation: RelationDragController; notify: (message: string) => void };

export function Gantt({ project, schedule, scale, selected, filter, focusUid, canFocus, onFocusChange, onSelect, onChange, onOrder, onSiblingOrder, onToggleCollapse, relation, notify }: Props) {
  ...
  const focus = useMemo(() => focusUid ? dependencyFocus(project, focusUid) : null, [project, focusUid]);
  ...
  const dependencyEdges = useMemo(() => displayDependencyEdges(dependencyProject, displayedUids), [dependencyProject, displayedUids]);
  ...
  const dragRef = useRef<Drag | null>(null), resizeRef = useRef<{ x: number; width: number } | null>(null), panRef = useRef<{ x: number; scrollLeft: number; moved: boolean } | null>(null), focusPositionRef = useRef<{ scrollLeft: number; windowX: number; windowY: number } | null>(null), skipClick = useRef(false), scroll = useRef<HTMLDivElement>(null);
  ...
  return <div className={`gantt-section ${focusUid ? 'dependency-focus-mode' : ''}`}>
    <div className="gantt-caption"><span>{dateString(origin)} — {dateString(end - 1)}</span><span>{focusUid ? `正在查看「${project.tasks.find(task => task.uid === focusUid)?.name}」的直接依赖 · 只读 · 松开 M 恢复` : '右键拖动设置依赖 · R + 右键拖动设置父任务 · 悬浮按住 M 聚焦'}</span><button disabled={!!focusUid} onClick={() => { if (scroll.current) scroll.current.scrollLeft = Math.max(0, (dayNumber(todayLocal()) - origin) * ppd - 100); }}>定位今天</button></div>
    ...
    {hover && hoverTask && !focusUid && !drag && !relation.drag && !isParent(hoverTask) && <HoverCard task={hoverTask} scheduled={schedule.get(hover.uid)!} project={project} x={hover.x} y={hover.y} />}
  </div>;
}
...
function TaskLabel({ item, isParent, listWidth, selected, dimmed, locked, total, onHover, onSelect, onOrder, onSiblingOrder, onToggleCollapse }: { item: ReturnType<typeof ganttTreeLayout>['items'][number]; isParent: boolean; listWidth: number; selected: string | null; dimmed: boolean; locked: boolean; total: number; onHover: (value: { uid: string; x: number; y: number } | null) => void; onSelect: (uid: string) => void; onOrder: (uid: string, order: number) => void; onSiblingOrder: (uid: string, targetUid: string) => void; onToggleCollapse: (uid: string) => void }) {
  const task = item.task;
  const top = item.top + (isParent ? 4 : 6);
  const height = 30;
  return <div className={`gantt-row tree-task-cell ${isParent ? 'tree-parent-cell' : ''} ${selected === task.uid ? 'selected-row' : ''} ${dimmed ? 'muted-row' : ''}`} data-task-uid={task.uid} style={{ top, height, width: listWidth, paddingLeft: 8 + item.depth * 18, zIndex: item.depth + 6 }} onMouseEnter={e => onHover({ uid: task.uid, x: e.clientX, y: e.clientY })} onMouseLeave={() => onHover(null)} onDoubleClick={e => { if (locked || !isParent || (e.target as HTMLElement).closest('.row-order, .row-grip')) return; e.preventDefault(); onToggleCollapse(task.uid); }} onDragOver={e => { if (!locked) e.preventDefault(); }} onDrop={e => { if (locked) return; const uid = e.dataTransfer.getData('text/mission-uid'); if (uid) { e.preventDefault(); onSiblingOrder(uid, task.uid); } }}><span className="tree-branch">{isParent ? task.collapse_children ? '▸' : '▾' : item.depth ? '└' : ''}</span><span className="row-grip" draggable={!locked} onDragStart={e => { if (locked) return e.preventDefault(); e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/mission-uid', task.uid); }}>⠿</span><span className="row-number" title={task.uid}>{task.uid}</span><button className="task-title" disabled={locked} title={isParent ? '双击折叠或展开子任务' : undefined} onClick={() => { if (!locked) onSelect(task.uid); }}><strong>{task.name}</strong>{!isParent && <span className="task-assignee">{task.assignee || '未指定执行人'}</span>}</button><div className="row-order"><button aria-label={`上移 ${task.name}`} disabled={locked || task.order === 1} onClick={() => onOrder(task.uid, task.order - 1)}>↑</button><button aria-label={`下移 ${task.name}`} disabled={locked || task.order === total} onClick={() => onOrder(task.uid, task.order + 1)}>↓</button></div></div>;
}
