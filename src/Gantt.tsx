...
import { displayDependencyEdges, ganttTreeLayout } from './ganttLayout';
...
export function Gantt({ project, schedule, scale, selected, filter, onSelect, onChange, onOrder, onSiblingOrder, onToggleCollapse, relation, notify }: Props) {
  ...
  const dependencyEdges = useMemo(() => displayDependencyEdges(project, displayedUids), [project, displayedUids]);
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => { if (event.key === 'Escape' && dragRef.current) cancelSchedule(); };
    const blur = () => cancelSchedule();
    window.addEventListener('keydown', down); window.addEventListener('blur', blur);
    return () => { window.removeEventListener('keydown', down); window.removeEventListener('blur', blur); };
  }, []);
  ...
  return <div className="gantt-section">
    <div className="gantt-caption"><span>{dateString(origin)} — {dateString(end - 1)}</span><span>右键拖动设置依赖 · R + 右键拖动设置父任务 · 双击左侧父任务折叠</span><button onClick={() => { if (scroll.current) scroll.current.scrollLeft = Math.max(0, (dayNumber(todayLocal()) - origin) * ppd - 100); }}>定位今天</button></div>
    ...
    {hover && !drag && !relation.drag && <HoverCard task={tasks.find(task => task.uid === hover.uid)!} scheduled={schedule.get(hover.uid)!} project={project} x={hover.x} y={hover.y} />}
  </div>;
}
...
function TaskLabel({ item, isParent, listWidth, selected, dimmed, total, onSelect, onOrder, onSiblingOrder, onToggleCollapse }: { item: ReturnType<typeof ganttTreeLayout>['items'][number]; isParent: boolean; listWidth: number; selected: string | null; dimmed: boolean; total: number; onSelect: (uid: string) => void; onOrder: (uid: string, order: number) => void; onSiblingOrder: (uid: string, targetUid: string) => void; onToggleCollapse: (uid: string) => void }) {
  const task = item.task;
  const top = item.top + (isParent ? 4 : 6);
  const height = 30;
  return <div className={`gantt-row tree-task-cell ${isParent ? 'tree-parent-cell' : ''} ${selected === task.uid ? 'selected-row' : ''} ${dimmed ? 'muted-row' : ''}`} data-task-uid={task.uid} style={{ top, height, width: listWidth, paddingLeft: 8 + item.depth * 18, zIndex: item.depth + 6 }} onDoubleClick={e => { if (!isParent || (e.target as HTMLElement).closest('.row-order, .row-grip')) return; e.preventDefault(); onToggleCollapse(task.uid); }} onDragOver={e => e.preventDefault()} onDrop={e => { const uid = e.dataTransfer.getData('text/mission-uid'); if (uid) { e.preventDefault(); onSiblingOrder(uid, task.uid); } }}><span className="tree-branch">{isParent ? task.collapse_children ? '▸' : '▾' : item.depth ? '└' : ''}</span><span className="row-grip" draggable onDragStart={e => { e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/mission-uid', task.uid); }}>⠿</span><span className="row-number" title={task.uid}>{task.uid}</span><button className="task-title" title={isParent ? '双击折叠或展开子任务' : undefined} onClick={() => onSelect(task.uid)}><strong>{task.name}</strong>{!isParent && <span className="task-assignee">{task.assignee || '未指定执行人'}</span>}</button><div className="row-order"><button aria-label={`上移 ${task.name}`} disabled={task.order === 1} onClick={() => onOrder(task.uid, task.order - 1)}>↑</button><button aria-label={`下移 ${task.name}`} disabled={task.order === total} onClick={() => onOrder(task.uid, task.order + 1)}>↓</button></div></div>;
}
