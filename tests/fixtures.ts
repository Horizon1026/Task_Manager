import { type Project, type Task } from '../src/model';
...
export function project(tasks: Task[] = [task('a')]): Project {
  return { version: 1, project: { name: '测试项目', start_date: '2026-09-14', default_scale: 'week', theme: 'light', default_filter: { labels: [], mode: 'or' }, allow_assignee_parallel_tasks: true, assignees: ['未指定'] }, calendar: { years: [], overrides: [] }, tasks: tasks.map((t, i) => ({ ...t, order: i + 1 })) };
}
